type Translator = (key: string, options?: { defaultValue?: string; ns?: string }) => string;
type Exists = (key: string, options?: { ns?: string }) => boolean;
...
export function buildSupportOptions(
  ...
  return values.map((value) => {
    ...
    return {
      value,
      label: exists(translationKey, { ns: 'homeVisits' })
        ? t(translationKey, { ns: 'homeVisits' })
        : t(translationKey, { defaultValue: fallbackLabel, ns: 'homeVisits' }),
    };
  });
}
