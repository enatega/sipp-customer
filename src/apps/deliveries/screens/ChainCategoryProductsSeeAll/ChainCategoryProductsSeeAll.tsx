...
export default function ChainCategoryProductsSeeAll() {
  const { colors } = useTheme();
  const { t } = useTranslation('deliveries');
  const navigation = useNavigation<NavigationProp>();
  const route =
    useRoute<RouteProp<DeliveriesStackParamList, 'ChainCategoryProductsSeeAll'>>();
  ...
  const {
    ...
    fetchNextPage,
    isRefetching,
  } = useChainCategoryProducts(categoryId, {
    mode: 'paginated',
  });
  ...
  return (
    <View style={[styles.screen, { backgroundColor: colors.background }]}>
      <GenericFilterablePaginatedListScreen<DeliveryShopTypeProduct>
        ...
        emptyDescription={t('chain_category_products_empty')}
      />
    </View>
  );
}
